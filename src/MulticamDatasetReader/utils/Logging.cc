...
void MD::Log::debug(const message_type_t& msg, const std::string& line_ending){
	if(log_level >= log_level_value::debug){
		std::cout << "\u001b[30;1mDEBUG\u001b[0m: " << msg << line_ending;
	}
}
void MD::Log::info(const message_type_t& msg, const std::string& line_ending){
	if(log_level >= log_level_value::info){
		std::cout << "\u001b[37;1mINFO\u001b[0m: " << msg << line_ending;
	}
}

void MD::Log::warn(const message_type_t& msg, const std::string& line_ending){
	if(log_level >= log_level_value::warn){
		std::cout << "\u001b[33;1mWARN\u001b[0m: " << msg << line_ending;
	}
}

void MD::Log::error(const message_type_t& msg, const std::string& line_ending){
